...
public class NhanVienRepository{
    ...
    public List<NhanVienViewModel> getList(int trangThai) {
        ...
        String sql = "SELECT Id,Ten,GioiTinh ,NgaySinh,SDT,Email,DiaChi,MatKhau,TrangThai FROM NhanVien WHERE TrangThai LIKE ?";
        try {
    ...
    public boolean add(NhanVien nv) {
        String sql = "INSERT INTO NhanVien(Ten,GioiTinh,NgaySinh,Sdt,Email,DiaChi,TrangThai)VALUES (?,?,?,?,?,?,?)";
        int check = 0;
        try {
    ...
    public boolean update(String maNV, NhanVien nv) {
        String sql = "UPDATE NhanVien SET Ten = ?,GioiTinh = ?,NgaySinh = ?,SDT = ?,Email = ?,DiaChi = ?, TrangThai = ? WHERE Id LIKE ?";
        int check = 0;
        try {
    ...
    public List<NhanVienViewModel> getSearch(String tenNV, String sdt, int trangThai) {
        List<NhanVienViewModel> listNV = new ArrayList<>();
        String sql =  "SELECT  Id,Ten ,GioiTinh ,NgaySinh,SDT,Email,DiaChi,MatKhau,TrangThai FROM NhanVien WHERE (Ten LIKE ? OR SDT LIKE ?) AND TrangThai LIKE ?";
        try {
